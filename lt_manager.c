...
static void _manager_info(int type)
{
	...
	else if(type == MANAGER_INTRO)
	{
		...
		rt_kprintf("LtMotorLib support finsh to help you control motors !\n ");
	...
	else if(type == MANAGER_HELP)
	{
		...
		rt_kprintf("ltmotorlib motor s_curve step freq_max freq_min flexible \n");
		rt_kprintf("ltmotorlib motor line_interp y_motor x_pos, y_pos \n");
		rt_kprintf("ltmotorlib motor circular_interp y_motor x_start y_start x_end y_end dir(CW/CCW) \n");
		rt_kprintf("******************* stepper specified part ************************\n\n");
...
static void _motor_stepper_part(int argc,char *argv[],lt_motor_t motor, struct lt_motor_info* info)
{
	...
	else if(!rt_strcmp(argv[2],"s_curve"))
	{
		if(argc == 7)		/* check paramaters */
		{
			...
			test_stepper_s_curve(motor,step,max,min,flex);
			flag = 1;
		}
	}
	else if(!rt_strcmp(argv[2],"line_interp") || !rt_strcmp(argv[2],"circular_interp"))
	{
		...
		/* check params*/
		if(argc == 6)		/* check paramaters, line interp */
		{
			int x_pos = atoi(argv[4]);
			int y_pos = atoi(argv[5]);
			test_stepper_line_interp(motor,y_motor,x_pos,y_pos);
			flag = 1;
		}
		else if(argc == 9)	/* check paramaters, circular interp */
		{
			int x_start = atoi(argv[4]);
			int y_start = atoi(argv[5]);
			int x_end = atoi(argv[6]);
			int y_end = atoi(argv[7]);
			rt_uint8_t dir = DIR_UNKNOWN;
			if(!rt_strcmp(argv[8],"CW"))
			{
				dir = DIR_CW;
				flag = 1;
			}
			else if(!rt_strcmp(argv[8],"CCW"))
			{
				dir = DIR_CCW;
				flag = 1;
			}
			test_stepper_circular_interp(motor,y_motor,x_start,y_start,x_end,y_end,dir);
		}
	}
	if(!flag) _manager_info(MANAGER_UNVALID);
}
...
static void ltmotorlib(int argc, char*argv[])
{
	...
	/* check command */
	if(!rt_strcmp(argv[2],"output") || !rt_strcmp(argv[2],"output_angle") || !rt_strcmp(argv[2],"get_pos") || !rt_strcmp(argv[2],"get_vel"))
	{	/* motor basic part*/
