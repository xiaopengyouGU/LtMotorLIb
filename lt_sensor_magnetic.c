...
#define _TF				0.001	/* unit: s */
#define MAGNET_ADDR		0x36
#define HIGH_BYTE		0x0C
#define LOW_BYTE		0x0D

#define MAGNET_DIR		1		/* 0: value increase cw, 1: value increase ccw */
...
static float _magnetic_get_curr(lt_sensor_t sensor)
{
	...
	rt_uint8_t buf[1];					/* 12 bits: 0~4095 */
	...
	curr_val  =  two_byte * 2 * PI/4096;						/* transform to rad */
...
static float _magnetic_get_pos(lt_sensor_t sensor)
{
	...
	sensor->count = _get_count(curr,sensor->last,sensor->count,MAGNET_DIR);
	sensor->last = curr;
	position =  sensor->count*2*PI + curr;
